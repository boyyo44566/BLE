...
double a=0;
double x=0;
double y=0; //¦b§PÂ_NOTIFY ¶}±Ò«á +1



char str[32] = {0}; 

#define GATT_CLIENT_CFG_NOTIFY           0x0001 //¥´¶}notify¶}Ãöªº¼Æ­È 
#define GUA_CHAR4_Hdl                           0x0027 //char4ªº¥y¬`   
#define GUA_CHAR4_CCC_Hdl                       0x28 //char4ªºCCCªº¥y¬`   
...
static void SimpleBLECentral_processRoleEvent(gapCentralRoleEvent_t *pEvent)
{
  ...
  {
    case GAP_DEVICE_INIT_DONE_EVENT:    //START_DEVICE_EVT¨Æ¥ó³B²z§¹¡A³]³Æªì©l¤Æ¦¨¥\´N·|²£¥Í
      {
    ...
    case GAP_DEVICE_INFO_EVENT:   //±½´y
      {
        // if filtering device discovery results based on service UUID
        
        if (DEFAULT_DEV_DISC_BY_SVC_UUID == TRUE)  //¦¹®ÉpEvent->deviceInfo.pEvtData´N¬OPeripheralµoªºadvertData¡Alen´N¬OadvertDataªºªø«× 
        {                                          //¤U­±³o­Ó¨ç¼Æ´N¬O¸ÑªRPeripheralµo°eªºadvertData³o­Ó¦r²Å¦ê¡A§ä¨ì»PSIMPLEPROFILE_SERV_UUID°t¤Çªº¶µ

          if (SimpleBLECentral_findSvcUuid(SIMPLEPROFILE_SERV_UUID,  //³q¹LUUID§ä¨ì³]³Æ
                                           pEvent->deviceInfo.pEvtData,
                                           pEvent->deviceInfo.dataLen))
          {
             //µM¦Z§âPeripheralªº³]³Æ¦a§}¦s¨ìsimpleBLEDevList¤¤
        ...
        a =   pEvent->deviceInfo.rssi;  //±µ¦¬RSSI
      }
      break;
      
    case GAP_DEVICE_DISCOVERY_EVENT:  //±½´yµ²§ô«áªñ¨Ó
      {
        // discovery complete
...
PIN_setOutputValue(hSbpPins,Board_LED1,1); 
         ...
         break;*/
   /* case GAP_END_DISCOVERABLE_DONE_EVENT: //±½´y¼Æ¨ì¹F«á¶i¤J
      {
      ...
      case GAP_LINK_ESTABLISHED_EVENT: //³o­Ó¨Æ¥ó¬O«ö¤U¤¤Áäªº³B²z¨ç¼Æ°õ¦æ§¹«á´N·|²£¥Í
      {
        if (pEvent->gap.hdr.status == SUCCESS)
        {
          ...
          if (charHdl == 0)   //©w®É¾¹¨ç¼Æ¡A²£¥Í¤@­ÓSTART_DISCOVERY_EVT¨Æ¥ó¡A·|¦bSimpleBLECentral_ProcessEvent¤¤³B²z
          {
  ...
  if (keys & KEY_UP)
  {
             ...
             procedureInProgress == FALSE)
    {
      ...
      if (doWrite)
      {
        // Do a write
        attWriteReq_t req;
     //   attWriteReq_t  lin_notify;

        req.pValue = GATT_bm_alloc(connHandle, ATT_WRITE_REQ, 1, NULL);
    //    lin_notify.pValue = GATT_bm_alloc(connHandle, ATT_WRITE_REQ, 1, NULL);
        
     //  if ( req.pValue != NULL )
        
        
        if ( y<1 )  //¥u¶i¨Ó¤@¦¸,¶}±Ònotify
        {

          req.handle = GUA_CHAR4_CCC_Hdl;  //char4ªºCCCªº¯S©º­È¥y¬`¡A³o¸Ì¼g¦º0x28¡A¥i³q¹LbtoolÀò¨ú¨ì  
          req.len = 2;  
          req.pValue[0] = LO_UINT16(GATT_CLIENT_CFG_NOTIFY); //0x0001¬°¶}±Ònotify  
      ...
      else
      {
       ...
       
       {
             ...
             lin_notify.handle = 0X0024;
             lin_notify.len = 7;
          
             lin_notify.pValue[0] =0x11;
             lin_notify.pValue[1] =0x22;
             lin_notify.pValue[2] =0x33;
             lin_notify.pValue[3] =0x44;
             lin_notify.pValue[4] =0x55;
             lin_notify.pValue[5] =0x66;
             lin_notify.pValue[6] =0x77;
             lin_notify.sig = 0;
             lin_notify.cmd = 0;
             status = GATT_WriteCharValue(connHandle, &lin_notify, selfEntity);  
  ...
  if (keys & KEY_SELECT)
  {
    ...
    if (state == BLE_STATE_IDLE)
    {
      ...
      {  //¥¼³s±µ«h«Ø¥ß³s±µ
        ...
        GAPCentralRole_EstablishLink(DEFAULT_LINK_HIGH_DUTY_CYCLE,   //  TRUE¬°°ª¦ûªÅ¤ñ±½´y¡A§_«h¬°FALSE
                                     DEFAULT_LINK_WHITE_LIST,        //  determines use of the white list 
                                     addrType, peerAddr);            //  addrTypePeer - address type of the peer device: 
...
static void SimpleBLECentral_processGATTMsg(gattMsgEvent_t *pMsg)
{
  if (state == BLE_STATE_CONNECTED)
  {
    ...
    //²K¥[±µ¦¬³qª¾¼ÆÕuªº³B²z³¡¤À¥Nº¿
    else if ( ( pMsg->method == ATT_HANDLE_VALUE_NOTI ) )   //³qª¾  
  {  
   if( pMsg->msg.handleValueNoti.handle == GUA_CHAR4_Hdl)     //CHAR6ªº³qª¾  ¦ê¤f¥´¦L  
   {
       // char str[32] = {0};  
